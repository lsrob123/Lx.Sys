        ...
        public virtual bool IsAuthorized(IAccessCriteria criteria, IdentityDto actualUser) {
            ...
            foreach (var role in actualUser.Roles) {
                if (role.RoleProcesses == null) {
                    isAuthorized = IsAuthorized(criteria, role.RoleType.Name, actualUser.UserReference);
                    if (isAuthorized)
                        break;
                ...
                foreach (var process in role.RoleProcesses) {
                    isAuthorized = IsAuthorized(criteria, role.RoleType.Name, actualUser.UserReference, process.Name,
